...
#
#   Franchie.
#   Roulette inspired strategy. Start with 1 unit, increment on each loss, reset after 1 consecutive same unit size win.
...
module Strategies
  module Steps
    module Strategy62XX1
      ...
      # 1 Trade & 1 Order.
      # Create a same side order to open at current trade take profit level.
      def step_6
        return false if oanda_active_trades.size == 0 && oanda_active_orders.size == 0 && reset_steps
        return false if oanda_active_trades.size == 0 && oanda_active_orders.size == 1 && exit_orders! && reset_steps
        return false if oanda_active_trades.size == 0 && oanda_active_orders.size == 2 && step_to(8) && queue_next_run
      ...
      def create_order_at_stop_loss_price!(type, trade)
        ...
        sequence = unit_size_sequence_index + 1

        case type.to_sym
        when :long
          units            = initial_units(:long) * unit_size_sequence[sequence]
          stop_loss_pips   = -channel_box_size_pips
          take_profit_pips = channel_box_size_pips
        when :short
          units            = initial_units(:short) * unit_size_sequence[sequence]
          stop_loss_pips   = channel_box_size_pips
          take_profit_pips = -channel_box_size_pips
        end
      ...
      def create_order_at_take_profit_price!(type, trade)
        ...
        case type.to_sym
        when :long
          units            = consecutive_wins < 1 ? initial_units(:long) * unit_size_sequence[sequence] : initial_units(:long)
          stop_loss_pips   = -channel_box_size_pips
          take_profit_pips = channel_box_size_pips
        when :short
          units            = consecutive_wins < 1 ? initial_units(:short) * unit_size_sequence[sequence] : initial_units(:short)
          stop_loss_pips   = channel_box_size_pips
          take_profit_pips = -channel_box_size_pips
        end
      ...
      def initial_units(type)
        ...
        units = (calculated_units_from_balance(config[:margin], type) || config[:units]) * unit_size_sequence[0]
      ...
      def increment_consecutive_wins
        super
        $redis.incr("#{key_base}:unit_size_sequence_index")
      end
      ...
      def reset_consecutive_losses
        super
        $redis.set("#{key_base}:unit_size_sequence_index", 0) if consecutive_wins >= 1
      end

      def cleanup
        reset_consecutive_wins
        reset_consecutive_losses
        $redis.del("#{key_base}:unit_size_sequence_index")
