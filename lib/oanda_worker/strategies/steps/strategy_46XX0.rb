...
#
#   H1 high_low - determine trend + buy and sell siganls.
#   D  close    - determine trend.
#
# To enter a trade:
#
#   H1 chart should give a buy or sell signal.
#   H1 trend should be in the same direction.
#   D trend should be in the same direction.
#
# To exit a trade:
#
#   Uses a stop loss order on the trade to exit on a sell signal.
#
module Strategies
  module Steps
    module Strategy46XX0
      # 0 Trades & 0 Orders.
      ...
      def step_2
        return false if oanda_active_trades.any? && oanda_active_orders.any? && step_to(4) && queue_next_run
        return false if oanda_active_trades.any? && oanda_active_orders.empty? && step_to(3) && queue_next_run
        return false if oanda_active_trades.empty? && oanda_active_orders.empty? && step_to(1) && queue_next_run
      ...
      def create_long_order!
        ...
        backtest_logging("#{points_a.last['xo']} #{points_a.last['xo_length']} [#{points_a.last['trend_length']}] - #{points_b.last['xo']} #{points_b.last['xo_length']} [#{points_b.last['trend_length']}]")

        return create_order_at!('long', order_price: xo_order_price, stop_loss_pips: xo_stop_loss_pips)
      end
      ...
      def create_short_order!
        ...
        return create_order_at!('short', order_price: xo_order_price, stop_loss_pips: xo_stop_loss_pips)
      end
      ...
      def update_long_order!(order)
        if current_xo_a == 'o'
          ...
          if xo_stop_loss_price < current_stop_loss_price || xo_order_price < current_order_price
            update_order_at!('long', order['id'], order_price: xo_order_price, stop_loss_price: xo_stop_loss_price)
          end
        end
      end
      ...
      def update_short_order!(order)
        if current_xo_a == 'x'
          ...
          if xo_stop_loss_price > current_stop_loss_price || xo_order_price > current_order_price
            update_order_at!('short', order['id'], order_price: xo_order_price, stop_loss_price: xo_stop_loss_price)
          end
        end
      end
      ...
      def update_long_trade!(trade)
        ...
        if current_xo_a == 'x'
          xo_stop_loss_price = (xos[-1].first['xo_price'] - 2 * box_size[0] * pip_size).round(round_decimal)
