...
#   D xo should be in the same direction.
#   RSI should be oversold when placing long order.
#   RSI should be overbought when placing short order.
#
# To exit a trade:
#
  ...
  module Steps
    module Strategy49XX1
      ...
      def create_long_order!(units = nil, tag = nil)
        ...
        backtest_logging("#{points_a.last['xo']} #{points_a.last['xo_length']} : #{points_a.last['trend']} #{points_a.last['trend_length']} - #{points_b.last['xo']} #{points_b.last['xo_length']} : #{points_b.last['trend']} #{points_b.last['trend_length']} - rsi: #{relative_strength_index.point.round(4)}")
      ...
      def create_long_order?
        ...
        return false if relative_strength_index.point > 20
      ...
      def create_short_order?
        ...
        return false if relative_strength_index.point < 80
      ...
      def current_candles
        @current_candles ||= candles(smooth: false, include_incomplete_candles: true, refresh: true)
      end
    end
  end
end
