...
module Strategies
  module Steps
    module Strategy8XXX0
      ...
      # If order immediately gets filled when created, we need to place target order 2.
      def step_7
        return false if oanda_active_trades.empty? && oanda_long_orders.size == 1 && oanda_short_orders.size == 0 && step_to(2) && queue_next_run
        return false if oanda_active_trades.empty? && oanda_long_orders.size == 0 && oanda_short_orders.size == 1 && step_to(3) && queue_next_run
        return false if oanda_active_trades.empty? && oanda_long_orders.size == 1 && oanda_short_orders.size == 1 && step_to(4) && queue_next_run
        ...
        if oanda_long_trades.any?
          if create_short_target_order!(1, oanda_long_trades.last)
            return false if step_to(9) && queue_next_run
          else
            return false if oanda_order['orderFillTransaction'] && oanda_order['orderFillTransaction']['tradesClosed'] && step_to(12) && queue_next_run
            return false if oanda_order['orderFillTransaction'] && oanda_order['orderFillTransaction']['tradeReduced'] && step_to(8) && queue_next_run
          end
        end
      ...
      # 1 Trade & 0 Orders.
      # Place target order 2.
      # This happens when target order 1 gets created and immediately gets filled.
      def step_8
        return false if oanda_active_trades.size == 1 && oanda_limit_orders.size == 2 && step_to(10) && queue_next_run
        return false if oanda_active_trades.size == 0 && oanda_active_orders.size >= 1 && step_to(12) && queue_next_run
        return false if oanda_active_trades.size == 0 && oanda_active_orders.size == 0 && backtest_export && reset_steps

        if oanda_long_trades.any?
          if create_short_target_order!(2, oanda_long_trades.last)
            return false if step_to(11) && queue_next_run
          else
            return false if oanda_order['orderFillTransaction'] && oanda_order['orderFillTransaction']['tradesClosed'] && step_to(12) && queue_next_run
          end
        end
        ...
        if oanda_short_trades.any?
          if create_long_target_order!(2, oanda_short_trades.last)
            return false if step_to(11) && queue_next_run
          else
            return false if oanda_order['orderFillTransaction'] && oanda_order['orderFillTransaction']['tradesClosed'] && step_to(12) && queue_next_run
          end
      ...
      def step_9
        ...
        if oanda_long_trades.any?
          return false if create_short_target_order!(2, oanda_long_trades.last) && step_to(10) && queue_next_run
        end

        if oanda_short_trades.any?
          return false if create_long_target_order!(2, oanda_short_trades.last) && step_to(10) && queue_next_run
        end

        false
      end

      # 1 Trade & 2 Target Orders.
      # Wait for stop loss to trigger.
      ...
      def step_10
        ...
        if oanda_active_trades.size == 1 && oanda_limit_orders.size == 1
          return false if roll_stop_to_break_even! && step_to(11) && queue_next_run
        end
      ...
      def step_12
        return false if oanda_active_trades.size == 1 && oanda_limit_orders.size == 2 && step_to(10) && queue_next_run
        return false if oanda_active_trades.size == 1 && oanda_limit_orders.size == 1 && step_to(11) && queue_next_run
        return false if oanda_active_trades.size == 0 && oanda_active_orders.size == 0 && backtest_export && reset_steps
