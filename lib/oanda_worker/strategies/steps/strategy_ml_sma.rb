module Strategies
  module Steps
    module StrategyMLSMA
      def step_1
        return true if self.oanda_trade = oanda_last_trade
        candles(count: prediction_candles_required)
        @simple_moving_average = Overlays::SimpleMovingAverage.new(candles: candles, count: 40).point
      ...
      def enter_long?
        ...
        simple_moving_average < candles['candles'][-1]['mid']['c'].to_f
      end
