  ...
  module Steps
    module Strategy74XX0
      ...
      def step_4
        ...
        backtest_logging("trades_opened: #{trades_opened}, trades_closed: #{trades_closed}, channel_box_size_pips: #{channel_box_size_pips}, take_profit_pips: #{take_profit_pips.round(1)}, max_xo_length: #{max_xo_length}, from_date: #{indicator_pf_b['data'].first['attributes']['candle_at'].split('T')[0]}")
      ...
      def enter_long?
        fractal.possible_down? && current_point_pf_a['trend'] == 'up' && current_point_pf_c['trend'] == 'up' && current_point_pf_d['trend'] == 'up'
      end

      def enter_short?
        fractal.possible_up? && current_point_pf_a['trend'] == 'down' && current_point_pf_c['trend'] == 'down' && current_point_pf_d['trend'] == 'down'
      end

      def exit_long?
        trades_opened >= auto_take_profit_after && current_point_pf_b['xo'] == 'o' && xos_b_since_first_trade.size >= 3
      end

      def exit_short?
        trades_opened >= auto_take_profit_after && current_point_pf_b['xo'] == 'x' && xos_b_since_first_trade.size >= 3
      end
      ...
      def max_xo_length
        @max_xo_length ||= indicator_pf_b['data'].map{ |point| point['attributes']['xo_length'] }.max
      end
      ...
      def current_point_pf_c
        @current_point_pf_c ||= indicator_pf_c['data'].last['attributes']
      end

      def current_point_pf_d
        @current_point_pf_d ||= indicator_pf_d['data'].last['attributes']
      end
      ...
      def points_pf_b
        @points_pf_b ||= indicator_pf_b['data'].map{ |point| point['attributes'] }
      end

      def points_pf_c
        @points_pf_c ||= indicator_pf_c['data'].map{ |point| point['attributes'] }
      end

      def points_pf_d
        @points_pf_d ||= indicator_pf_d['data'].map{ |point| point['attributes'] }
      end
      ...
      def indicator_pf_c
        @indicator_pf_c ||= begin
          results = { 'data' => [] }

          indicator_pf['data'].each_with_index do |data, i|
            next unless indicator_pf['data'][i]['attributes']['granularity'] == granularity[2].downcase &&
                        indicator_pf['data'][i]['attributes']['box_size'] == box_size[2] &&
                        indicator_pf['data'][i]['attributes']['reversal_amount'] == reversal_amount[2] &&
                        indicator_pf['data'][i]['attributes']['high_low_close'] == high_low_close[2]
      ...
      def indicator_pf_d
        @indicator_pf_d ||= begin
          results = { 'data' => [] }

          indicator_pf['data'].each_with_index do |data, i|
            next unless indicator_pf['data'][i]['attributes']['granularity'] == granularity[3].downcase &&
                        indicator_pf['data'][i]['attributes']['box_size'] == box_size[3] &&
                        indicator_pf['data'][i]['attributes']['reversal_amount'] == reversal_amount[3] &&
                        indicator_pf['data'][i]['attributes']['high_low_close'] == high_low_close[3]
      ...
      def xos_b
        @xos_b ||= xos(points_pf_b, keys: ['xo_price', 'xo_length', 'xo', 'trend'])
      end

      def xos_b_since_first_trade
        @xos_b_since_first_trade ||= begin
