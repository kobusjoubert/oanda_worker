module Strategies
  class Strategy41120 < Strategy
    ...
    def current_high
      @current_high ||= (candles['candles'][-1]['mid']['h'].to_f / PIP_SIZE).round
    end

    def current_low
      @current_low ||= (candles['candles'][-1]['mid']['l'].to_f / PIP_SIZE).round
    end

    def current_close
      @current_close ||= (candles['candles'][-1]['mid']['c'].to_f / PIP_SIZE).round
    end
    ...
    # It is important not to memoize becuase we ask for this value a second time after updating it in the same step.
    def last_box_price
      $redis.get("#{key_base}:last_box_price").to_i
    end
